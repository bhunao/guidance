    ...
    def process(self):
        ...
        for ent, (render, velocity) in self.world.get_components(AnimatedSpriteComponent, VelocityComponent):
            ...
            match Tiles(tile):
                ...
                case _:
                    pass
            render.x, render.y = new_pos


class AnimatedSpriteSystem(Processor):
    def __init__(self) -> None:
        super().__init__()
...
class KeyboardInputProcessor(Processor):
    ...
    def process(self):
        ...
        for ent, (camera) in self.world.get_components(CameraComponent):
            camera = camera[0]
        ...
        for ent, (velocity, player) in self.world.get_components(VelocityComponent, PlayerComponent):
            if pyxel.btn(pyxel.KEY_LEFT):
                velocity.x -= 1
            if pyxel.btn(pyxel.KEY_RIGHT):
                velocity.x += 1
